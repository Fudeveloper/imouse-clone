...
| Event/error contract | XP API/help 信号包含 `msgid`、callback、日志、错误、设备/分组/helper 域 | GUI `Events` 面板、callback ledger、Attach Log、Problems/Rerun/Recovery 关联 | GUI helper 测试和 `docs/xp_event_error_contract.md` | 真实 receiver/HID callback、XP 硬件错误、账号/授权错误和逐设备日志过滤器未验证 | 重跑前使用 Events；将 callback/日志/错误转化为 JSONL evidence 和阶段门 |
| Python SDK/helper | XP Python 页有 api/helper/console/device 分层 | `XpApiClient` 初版，覆盖设备、分组、校准、profile、图色、批量、callback list/push/clear，以及本地 ImConfig/User/Shortcut runtime helper | `tests/test_xp_client.py` | 真实 receiver/HID callback 事件、云用户/权限、快捷指令真实执行仍未验证 | P1 后接入真实事件 callback；P3 后再设计账号权限和快捷指令执行模型 |
...
## 2026-06-09 API client 变更

- `XpApiClient.screenshot_bytes()` 现已覆盖从 `/pic/screenshot` 以 `binary=true` 下载 XP 原始截图。
- `XpApiClient.screenshot(binary=True)` 保留为兼容快捷方式，通过同一路径返回原始字节。
- 这只证明本地 API/client 行为。不证明 receiver 采集质量、帧率、HID 响应或真实 iPhone 控制。
